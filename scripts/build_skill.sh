#!/usr/bin/env bash
# Build claude-ai/jerbs.skill from source files.
# The .skill format is a ZIP archive containing:
#   jerbs/SKILL.md              <- claude-ai/SKILL.md
...
OUT="$REPO_ROOT/claude-ai/jerbs.skill"
TMP="$(mktemp -d)"

trap 'rm -rf "$TMP"' EXIT

mkdir -p "$TMP/jerbs/scripts"
cp "$REPO_ROOT/claude-ai/SKILL.md"              "$TMP/jerbs/SKILL.md"
