...
def run_setup_wizard(output_path: Path):
    ...
    criteria = json.loads((Path(__file__).parent.parent / "shared" / "criteria_template.json").read_text())
    ...
    criteria["identity"]["background_summary"] = ask(
        "Background summary",
        "Experienced software engineer targeting senior roles"
    )
    criteria["identity"]["seniority_level"] = ask(
        "Seniority level",
        "Senior and above — no junior, mid-level, or intern"
    )
    criteria["identity"]["target_roles"] = ask_list(
        "Target roles",
        "Senior SRE, Staff Engineer, Principal Engineer"
    )
    ...
    criteria["target_companies"]["prestige_requirement"] = ask(
        ...
        "Upper-tier only — no unknown companies"
    )
    criteria["target_companies"]["whitelist"] = ask_list(
        "Dream companies (always flag positively, optional)",
        "Google, Anthropic, OpenAI"
    )
    criteria["target_companies"]["blacklist"] = ask_list(
        "Companies to always ignore",
        "Deloitte, Accenture, random staffing agencies"
    )
    criteria["target_industries_blocklist"] = ask_list(
        "Industries to block entirely (optional)",
        "defense, tobacco, gambling, MLM"
    )

    print("\n── Role requirements ─────────────────────────────────")
    ft_only = ask_bool("Full-time only?", True)
    criteria["role_requirements"]["employment_type"] = ["full-time"] if ft_only else ["full-time", "contract"]
    criteria["role_requirements"]["remote_preference"] = ask(
        "Remote preference",
        "Depends on location — must be disclosed"
    )

    print("\n── Compensation ──────────────────────────────────────")
    criteria["compensation"]["base_salary_floor"] = ask_int(
        "Minimum base salary ($)", 150000
    )
    criteria["compensation"]["base_salary_currency"] = ask("Currency", "USD")
    criteria["compensation"]["total_comp_target"] = ask_int(
        "Target total comp ($)", 250000
    )
    ...
    criteria["tech_stack"]["dealbreaker"] = ask_list("Stack dealbreakers (optional)", "Windows OS, Windows servers")
    ...
    criteria["profile_name"] = ask(
        "\nProfile name",
        f"{criteria['identity']['name']}'s Job Search" if criteria["identity"]["name"] else "My Job Search"
    )
    ...
    print(f"  Biz hours:  {criteria['search_settings']['biz_start_hour']}:00–"
