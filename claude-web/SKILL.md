...
description: >
  ...
  the LinkedIn MCP is connected. Screens Gmail in one unified pass (job alert digests +
...
level. Screens Gmail in one unified pass (plus an optional LinkedIn DM pass), applies the user's criteria, surfaces draft replies,
...
### Gmail pass — unified search

Run a **single** `gmail_search_messages` query that captures both digest alerts and direct
...
**Classify each message** as you read it — this determines source label and screening rules:

| Classification | Sender pattern | Source label | Notes |
|---|---|---|---|
| **Job Alert Digest** | `from:linkedin.com`, `jobalerts.indeed.com`, `indeedemail.com`, or other subscription alert senders | `Job Alert Listings` | Screen the **individual job listings** within each digest, not the email as a whole. "Generic mass email" dealbreaker does NOT apply — these are subscription alerts. |
| **Direct Outreach** | Everything else that passes noise filtering | `Direct Outreach` | Screen the email itself. Filter out non-job noise first (surveys, loyalty emails, newsletters, mailing list patches, government/non-profit announcements). Apply the "generic mass email" dealbreaker: no name, boilerplate, no reference to specific background = hard fail. |
