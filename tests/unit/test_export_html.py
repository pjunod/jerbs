...
class TestBuildCard:
    ...
    def test_sent_reply_shows_sent_label(self):
        ...
        assert "Sent — logged to correspondence log" in html
...
class TestExportToHtml:
    ...
    def test_page_has_title(self):
        ...
        assert "<title>Jerbs Results — 2026-04-02</title>" in html

    def test_page_has_css(self):
        html = run_html_export([])
        assert "<style>" in html
        assert "--bg:" in html

    def test_page_has_theme_toggle(self):
        html = run_html_export([])
        assert "theme-btn" in html
        assert "toggleTheme" in html

    def test_page_has_light_class_support(self):
        html = run_html_export([])
    ...
    def test_actions_section_at_top(self):
        ...
        # Find the "Results" section header (not just any occurrence of "Results")
        results_pos = html.index("Results</div>")
        assert action_pos < results_pos
    ...
    def test_fail_items_in_collapsible(self):
    ...
    def test_source_column_in_fail_table(self):
        items = [
            make_result("fail", source="Direct Outreach"),
            make_result("fail", source="Job Alert Listings"),
        ]
        html = run_html_export(items)
    ...
    def test_footer_present(self):
        html = run_html_export([], run_date="2026-04-02")
        assert "Generated by jerbs" in html
...
class TestConstants:
    ...
    def test_theme_js_not_empty(self):
