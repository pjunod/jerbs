...
VERDICT_BADGE_CLASS = {"pass": "badge-pass", "maybe": "badge-maybe", "fail": "badge-fail"}
...
.mode-badge {
  display: inline-block; background: #1a2332; border: 1px solid var(--blue);
...
.action-banner {
  ...
  border-radius: 0.5rem; padding: 1rem 1.25rem; margin-bottom: 2rem;
}
...
.comp-note {
  font-size: 0.8rem; padding: 0.25rem 0.5rem; background: #1a2332;
  border-radius: 0.25rem; display: inline-block; margin-top: 0.25rem;
}
.draft-block {
  margin-top: 0.5rem; padding: 0.75rem; background: #0d1117;
...
.fail-table td { padding: 0.5rem 0.75rem; border-bottom: 1px solid #21262d; vertical-align: top; }
...
details summary::before { content: '▶'; font-size: 0.65rem; transition: transform 0.2s; }
...
def build_card(item, verdict):
    ...
    comp_html = ""
    if comp:
        comp_html = f'<div class="comp-note">{_e(comp)}</div>'
...
def build_fail_table(items):
    ...
    for item in items:
        ...
        location = _e(item.get("location", ""))
        reason = _e(item.get("reason", ""))
        dealbreaker = item.get("dealbreaker", "")
        email_url = item.get("email_url", "")

        company_cell = _link(email_url, company) if email_url else company
        reason_class = (
            "reason-col blacklist" if "blacklist" in dealbreaker.lower() else "reason-col"
        )
        rows.append(
            f"<tr><td>{company_cell}</td><td>{role}</td>"
            f'<td>{location}</td><td class="{reason_class}">{reason}</td></tr>'
        )

    return (
        '<table class="fail-table"><thead>'
        "<tr><th>Company</th><th>Role</th><th>Location</th><th>Reason</th></tr>"
        "</thead><tbody>" + "\n".join(rows) + "</tbody></table>"
    )


def build_pass_section(items_by_verdict, section_label):
    """Build a full section (Job Alerts / Direct Outreach / LinkedIn DMs)."""
    passes = items_by_verdict.get("pass", [])
    maybes = items_by_verdict.get("maybe", [])
    fails = items_by_verdict.get("fail", [])

    if not passes and not maybes and not fails:
        return ""

    html = '<div class="section">'
    html += f'<div class="section-header">{_e(section_label)}</div>'

    if passes:
        html += '<div class="verdict-label pass">Interested</div>'
        for item in passes:
            html += build_card(item, "pass")

    if maybes:
        html += '<div class="verdict-label maybe" style="margin-top: 1.5rem;">Maybe</div>'
        for item in maybes:
            html += build_card(item, "maybe")

    if fails:
        html += '<div style="margin-top: 1.5rem;">'
        html += "<details>"
        html += f"<summary>Filtered out &middot; {len(fails)} listings</summary>"
        html += build_fail_table(fails)
        html += "</details></div>"
...
def export_to_html(results_data, output_path):
    ...
    # Group items by pass (source) then verdict
    sections = {}
    for item in results_data.get("results", []):
        source = item.get("source", "Job Alert Listings")
        verdict = item.get("verdict", "fail")
        sections.setdefault(source, {}).setdefault(verdict, []).append(item)

    # Count totals
    counts = {"pass": 0, "maybe": 0, "fail": 0}
    for item in results_data.get("results", []):
        counts[item.get("verdict", "fail")] += 1
...
<title>Jerbs Results — {_e(run_date)}</title>
<style>{CSS}</style>
</head>
<body>

<h1>Jerbs Results <span class="mode-badge">{_e(mode_label)}</span></h1>
<p class="subtitle">{_e(run_date)} &middot; {_e(lookback)}-day lookback &middot; {_e(profile_name)}</p>
...
"""
    ...
    section_labels = {
        "Job Alert Listings": "Job Alert Listings &middot; Pass 1",
        "Direct Outreach": "Direct Outreach &middot; Pass 2",
        "LinkedIn DMs": "LinkedIn DMs &middot; Pass 3",
    }

    for source in section_order:
        if source in sections:
            html += build_pass_section(sections[source], section_labels.get(source, source))

    # Any other sources not in the standard order
    for source in sections:
        if source not in section_order:
            html += build_pass_section(sections[source], source)

    html += f"""
<footer>
  Generated by jerbs &middot; {_e(run_date)} &middot; {_e(mode_label)} mode
</footer>
