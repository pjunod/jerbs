...
DEAD_END_STATUSES = {
    "Offer declined", "No response", "Withdrew", "Rejected", "Filtered out"
}
...
def export_to_xlsx(results_data, output_path):
    ...
    ordered_dead_end_groups = [(s, dead_end_groups[s]) for s in dead_end_order if s in dead_end_groups]
    ...
    wb.save(output_path)
    print(f"Exported {len(items)} results → {len(active_items)} active, {len(dead_end_items)} collapsed | {output_path}")
